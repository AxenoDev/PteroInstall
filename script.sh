...
if [[ "$PANEL" = 'pterodactyl' && "$OS" = 'Centos 7'  ]]; then
  ...
  yum -y update
  yum install -y curl
echo -e "${rouge}Erreur - System non supporter ${neutre}"
...
#Debian 11
#========
elif [[ "$PANEL" = 'pterodactyl' && "$OS" = 'Debian 11' ]]; then
  echo "Pterodactyl + Debian 11"
 apt update
 apt install -y curl
bash <(curl https://ressources.bagou450.com/scripts/debian10/Debian10ptero.sh)
sleep 2
elif [[ "$PANEL" = 'mineweb' && "$OS" = 'Debian 11' ]]; then
  echo "MineWeb + Debian 11"
 apt update
 apt install -y curl
bash <(curl https://ressources.bagou450.com/scripts/debian10/Debian10mineweb.sh)
sleep 2
elif [[ "$PANEL" = 'pufferpanel' && "$OS" = 'Debian 11' ]]; then
  echo "PufferPanel + Debian 11"
 apt update
 apt install -y curl
bash <(curl https://ressources.bagou450.com/scripts/debian1/Debian10pufferpanel.sh)
sleep 2
elif [[ "$PANEL" = 'azuriom' && "$OS" = 'Debian 11' ]]; then
  echo "Azuriom + Debian 11"
 apt update
 apt install -y curl
